    ...
    return {
        async createUser(req, res) {
            ...
            const joiSchema = Joi.object({
                name: Joi.string().min(2).required()/* .regex(new RegExp('^[a-zA-Z](3-30)$')) */,
                email: Joi.string().min(2).email({ minDomainSegments: 2, tlds: { allow: ['com', 'net', 'in'] } }).required(),
                tel: Joi.string().min(2).required()/* .regex('^[0-9](8-12)$') */,
